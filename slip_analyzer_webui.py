# prop_analyzer_parlay_v16_final.py
...
def get_nfl_data():
    """
    Fetches all player, team, and schedule data.
    """
    ...
    rename_map = {'player_display_name': 'Player', 'team': 'team_abbr', 'opponent_team': 'Opponent', 'season': 'Year', 'week': 'Week', 'receptions': 'Rec', 'receiving_yards': 'Rec_Yds', 'rushing_yards': 'Rush_Yds', 'passing_yards': 'Pass_Yds', 'passing_tds': 'Pass_TD', 'rushing_tds': 'Rush_TD', 'receiving_tds': 'Rec_TD', 'completions': 'Pass_Cmp'}
    full_df.rename(columns=rename_map, inplace=True)
    full_df['TD'] = full_df.get('Rec_TD', 0) + full_df.get('Rush_TD', 0)
...
def calculate_probability(player_df, stat_col, prop_line, matchup_multiplier=1.0):
    ...
    if stat_col in ['TD', 'Rec_TD', 'Rush_TD', 'Pass_TD', 'Rec', 'Pass_Cmp']:
...
def add_to_parlay(player, stat_desc, stat_col, line, over_under, player_df, opponent, defense_df, team_map):
    ...
    if 'Rush' in stat_col:
        ...
        if not multiplier_series.empty: multiplier = multiplier_series.iloc[0]
    elif 'Pass' in stat_col or 'Rec' in stat_col:
...
if 'data_loaded' in st.session_state:
    ...
    if app_mode == "Single Prop Analysis":
        ...
        player_options = sorted(full_df[full_df['Player'].notna()]['Player'].unique())
        selected_player = st.sidebar.selectbox("Select Player", options=player_options, key="player_single", index=None, placeholder="Select a player...")

        stat_map = {"Receiving Yards": "Rec_Yds", "Rushing Yards": "Rush_Yds", "Passing Yards": "Pass_Yds", "Receptions": "Rec", "Total Touchdowns": "TD"}
        ...
        # **NEW**: Slider for recent games table
        ...
        if 'analysis_player' in st.session_state and st.session_state.analysis_player == selected_player:
            ...
            if player_df_single.empty or not stat_col or stat_col not in player_df_single.columns:
                ...
                with analysis_col:
                    ...
                    d_rank_label, d_avg_label = "D-Rank", "Avg Yds Allowed"
                    if 'Rush' in stat_col:
                    ...
                    # **MODIFIED**: Calculate both hit rates, independent of a radio button
                ...
                # **MODIFIED**: Replace single button with two buttons in columns
                ...
                final_chart = line + prop_line_rule + prop_line_text + opp_avg_rule + opp_avg_text
                st.altair_chart(final_chart, use_container_width=True)

                st.subheader(f"Recent Games")
                # **MODIFIED**: Use slider value `recent_n` to determine how many games to show
        ...
        # Parlay calculator code remains the same...
        st.sidebar.header("Add a Prop to Parlay")
        parlay_player_options = sorted(full_df[full_df['Player'].notna()]['Player'].unique())
        parlay_player = st.sidebar.selectbox("Player", options=parlay_player_options, key="player_parlay", index=None)
        parlay_stat_map = {"Receiving Yards": "Rec_Yds", "Rushing Yards": "Rush_Yds", "Passing Yards": "Pass_Yds", "Receptions": "Rec", "Total Touchdowns": "TD"}
