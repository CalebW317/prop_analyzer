# improved_underdog_dashboard.py
import os
import json
import requests
import pandas as pd
...
# -------------------------------
# STEP 8: Append Only Current Week to Master CSV
# -------------------------------
if master_rows:
    weekly_df = pd.concat(master_rows, ignore_index=True)
    weekly_df.to_csv(MASTER_CSV, mode='a', index=False, header=not Path(MASTER_CSV).exists())
